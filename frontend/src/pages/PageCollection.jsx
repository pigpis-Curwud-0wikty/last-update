...
const PageCollection = () => {
  ...
  const collectionType = type ? type : 'denim';
  const displayName = collectionType.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase());

  let filteredProducts = products.filter(
    (item) =>
      item.category &&
      normalize(item.category) === normalize(collectionType)
  );
...
>>>>>>> f928bb6 (last update)
          ...
          <p className="col-span-full text-center text-gray-400">No {collectionType} products found.</p>
        )}
      </div>
